...
const staticDir = process.env.STATIC_DIR ?? `/var/www/blst` // TODO
const ingestDir = process.env.INGEST_DIR ?? `/var/www/blog` // TODO

//}}}
//{{{ Core data structures

type DocName = {
   name: string // like `topic/subtopic`
}
...
class Blog {
   ...
   private deps: Map<string, ModDeps>;
   private cleanup: Cleanup[];
   constructor(private fs: FileSys){
      this.updated = new Date();
      this.navTopic: NavTree = {};
      this.navTime: NavTree = {};
      this.deps = []
      this.cleanup = []
   }
   ...
   checkNUpdate() {
      ...
      this.docCache.ingestAndRefresh()
      const navTrees = buildNavTrees()
      this.navTopic = navTrees.f1
      this.navTime = navTrees.f2
      this.updated = new Date()
   }

   async start(): Promise<Blog> {
   ...
   updateMedia() {
      /// Regenerate just this one document, update it in doc cache, add media to cleanup
      /// list
...
class DocCache {
   ...
   ingestAndRefresh() {
      ...
      console.log(`ingesting and refreshing at staticDir $staticDir`)
...
async function ingestFiles(staticDir: string, docCache: DocCache):
      Promise<[Ingested[], IngestedCore]> {
   ...
   const ingestCorePath = staticDir + ingestCoreSubfolder
   const lengthCorePrefix = ingestCorePath.length
   const ingestedCore = ingestCoreFiles(staticDir, ingestCorePath, docCache, lengthCorePrefix)

   const ingestPath = staticDir + ingestSubfolder
   ...
   const mediaDir = File(staticDir + mediaSubfolder)
   if (!mediaDir.exists()) { mediaDir.mkdirs() }
   const docDir = File(staticDir + docsSubfolder)
   if (!docDir.exists()) {docDir.mkdirs()}

   moveMediaFiles(mediaFiles, lengthPrefix)
   ingestScripts(scriptNames, docCache, false)
   moveDocs(docFiles, staticDir, ingestSubfolder, docsSubfolder)
...
async function ingestCoreFiles(ingestCorePath: string, docCache: DocCache, lenPrefix: number):
   ...
   const js = await moveFile(ingestCorePath, staticDir + scriptsSubfolder + globalsSubfolder, `core.js`)
   const css = await moveFile(ingestCorePath, staticDir + mediaSubfolder + globalsSubfolder, `core.css`)
   const favicon = await moveFile(ingestCorePath, staticDir + mediaSubfolder, `favicon.ico`)
...
async function readCachedCore(cache: DocCache) {
   const ingestDirN = staticDir + coreSubfolder
...
async function readCachedScripts(docCache: DocCache) {
   const scriptDirN = staticDir + scriptsSubfolder
